...
export class KarmaService {
    ...
    async processRepository({ owner, repo }: { owner: string, repo: string }, octokit: InstanceType<typeof GitHub>): Promise<void> {
        try {
            ...
            throw new Error(`Failed to process repository ${owner}/${repo}: ${(error as Error).message}`);
        }
    }
}
