...
describe('KarmaService', () => {
    ...
    describe('isExcluded', () => {
        beforeEach(() => {
            karmaService = new KarmaService({}, ['bot*', 'test.user', 'user@example.com']);
        });
        ...
        it('should not exclude non-matching usernames or emails', () => {
            expect(karmaService.isExcluded('normaluser', 'normal@example.com')).toBe(false);
        });
    });
    ...
    describe('processRepository', () => {
        ...
        beforeEach(() => {
            ...
            vi.spyOn(githubService, 'getPullRequestReviews').mockImplementation((params: any) => {
        ...
        it('should throw error when processing repository fails', async () => {
            vi.spyOn(githubService, 'getRepositoryPullRequests').mockRejectedValue(new Error('API error'));
            
            karmaService = new KarmaService({ pull_request: 5 }, []);
            
            await expect(karmaService.processRepository(repo, octokit)).rejects.toThrow(
