...
describe("run function", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(generateMarkdownModule.generateMarkdown).mockReturnValue(
      mockMarkdown,
    );
  ...
  it("should process a single repository when no organization is provided", async () => {
    ...
    expect(core.info).toHaveBeenCalledWith(
      "Run in repository mode: test-owner/test-repo",
    );
  ...
  it("should process organization repositories when organization is provided", async () => {
    ...
    vi.mocked(githubService.getOrganizationRepositories).mockResolvedValue(
      mockRepos as any,
    );
    ...
    expect(core.info).toHaveBeenCalledWith(
      "Run in organization mode: test-org",
    );
    expect(core.info).toHaveBeenCalledWith(
      "Found 2 repositories in organization test-org",
    );
  });
  ...
  it("should fail if token is not provided", async () => {
    ...
    expect(core.setFailed).toHaveBeenCalledWith(
      "Action failed: Token is required",
    );
  });
  ...
  it("should use provided karma config when available", async () => {
    ...
    expect(core.info).toHaveBeenCalledWith(
      `Karma config: ${JSON.stringify(customConfig)}`,
    );
  });
    ...
    vi.mocked(core.getInput).mockImplementation((name) => {
      switch (name) {
        case "token":
          return "mock-token";
        case "exclude":
          return JSON.stringify(excludeList);
        default:
          return "";
      }
    });
  ...
  it("should use markers when provided", async () => {
    ...
    expect(core.info).toHaveBeenCalledWith(
      "Using custom markers: <!-- START --> and <!-- END -->. This will overwrite the file content between these markers.",
    );
  });
});
