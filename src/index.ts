...
export async function run() {
    try {
        ...
        const karmaService: InstanceType<typeof KarmaService> = new KarmaService(karmaConfigParsed, excludeParsed)

        const organization: string | null = getInput('organization') || null
        if (organization) {
            info(`Run in organization mode: ${organization}`)

            const orgRepos: { repo: string; owner: string }[] = await getOrganizationRepositories({ org: organization, type: 'public' }, octokit)
                .then((repositories) => repositories.map(repo => ({ repo: repo.name, owner: organization })))

            info(`Found ${orgRepos.length} repositories in organization ${organization}`)

            await Promise.all(orgRepos.map(async ({ repo, owner }) => 
        ...
        let leaderboard: { username: string; points: number }[] = karmaService.getLeaderboard()
        ...
        if (marker_start && marker_end) {
            info(`Using custom markers: ${marker_start} and ${marker_end}. This will overwrite the file content between these markers.`)
            writeMarkdown(output, markdown, { marker_start, marker_end })
        } else {
            info('No markers provided, writing markdown to file. This will overwrite the file content.')
